...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            allPossible[jailed_position] = 1.0
        else:
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        nPositions = len(self.legalPositions)
        particles = list()
        for i in range(self.numParticles):
            pos = self.legalPositions[i % nPositions]
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        noisyDistance = observation
        emissionModel = busters.getObservationDistribution(noisyDistance) # P(e|X)
        pacmanPosition = gameState.getPacmanPosition()

        if noisyDistance is None:
            # update all particles so that ghost appears in prison cell
            jailed_position = self.getJailPosition();
            self.particles = [jailed_position for _ in range(self.numParticles)]
            ...
            for p in self.particles:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                particleWeights[p] += emissionModel[trueDistance] #update weight with true distance
            # resample
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        particles = [combinations[i % numCombinations] for i in range(numCombinations)]
        self.particles = particles
    ...
    def observeState(self, gameState):
        ...
        particleWeights = util.Counter()
        for p in self.particles:
            pWeight = 1.0 #set as float 1 instead of checking for 
            for i, ghost in enumerate(p):
                if noisyDistances[i] is not None: #skip if in jail, weight will be updated to 1 bc we know where it is
                    trueDistance = util.manhattanDistance(ghost, pacmanPosition)
                    pWeight *= emissionModels[i][trueDistance]
            particleWeights[p] += pWeight
        # resample
        if particleWeights.totalCount() == 0:
            self.initializeParticles()
        else:
            items = sorted(particleWeights.items())
            values, distribution = map(list, zip(*items)) #unzip into two lists
            samples = util.nSample(distribution, values, self.numParticles)
            # reset if ghost in jail because new sample for ghost doesn't matter
            if None in noisyDistances:
                for i in range(self.numGhosts):
                    if noisyDistances[i] is None:
                        samples = [self.getParticleWithGhostInJail(s,i) for s in samples]
            self.particles = samples
